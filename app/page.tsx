...
import { 
  ...
  PieChart, List, TrendingUp, Upload
} from "lucide-react";
...
const CATEGORY_LABELS: Record<string, string> = {
  moradia: 'Moradia', alimentacao: 'Alimentação', transporte: 'Transporte', contas: 'Contas', saude: 'Saúde', lazer: 'Lazer', trabalho: 'Trabalho', outros: 'Outros'
};

export default function Home() {
  const [loading, setLoading] = useState(true);
  const [transactions, setTransactions] = useState<any[]>([]);
  const [historyData, setHistoryData] = useState<any[]>([]); // Dados para o gráfico
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = async (event) => {
      ...
      // Pula cabeçalho (linha 0) e processa o resto
      for (let i = 1; i < lines.length; i++) {
        const line = lines[i].trim();
        if (!line) continue;
        
        // Esperado: data (YYYY-MM-DD), descricao, valor, categoria, tipo (expense/income)
        ...
        if (date && description && amount) {
          payloads.push({
            ...
            category: (category?.trim() || 'outros').toLowerCase(),
            type: (type?.trim() || 'expense').toLowerCase(),
            status: 'paid', // Assume pago se veio do histórico
            is_recurring: false
          });
        }
      }
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // 2. Dados Históricos (Últimos 6 meses para o Gráfico)
      const sixMonthsAgo = new Date();
      sixMonthsAgo.setMonth(sixMonthsAgo.getMonth() - 5);
      const startOfHistory = new Date(sixMonthsAgo.getFullYear(), sixMonthsAgo.getMonth(), 1).toISOString();
      
      const { data: historyTxs } = await supabase
        .from('transactions')
        .select('date, amount, type')
        .gte('date', startOfHistory)
        .order('date', { ascending: true });
      ...
      historyTxs?.forEach(t => {
          ...
          if (t.type === 'income') chartMap[key].income += Number(t.amount);
          else chartMap[key].expense += Number(t.amount);
      });
      // Converte para array ordenado
      const chartData = Object.entries(chartMap).sort().map(([date, val]) => ({
          label: date.split('-')[1], // Mês
          ...val
      }));
  ...
  const sortedCategories = Object.entries(byCategory).sort(([, a]: any, [, b]: any) => b - a).map(([cat, val]: any) => ({
      cat, val, percent: totalExpense > 0 ? (val / totalExpense) * 100 : 0
  }));

  // --- FUNÇÃO AUXILIAR PARA O GRÁFICO (Normalização) ---
  const maxChartVal = Math.max(...historyData.map(d => Math.max(d.income, d.expense)), 100);
  const getH = (val: number) => Math.max((val / maxChartVal) * 100, 2); // Altura em %
  ...
  return (
    <div className="flex h-screen overflow-hidden relative z-10 text-zinc-100 font-sans">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-14 border-b border-tactical bg-surface/80 backdrop-blur flex items-center justify-between px-4 md:px-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
                ...
                <button className="flex items-center gap-2 px-3 py-1.5 bg-zinc-900 border border-zinc-800 rounded hover:border-zinc-600 transition-colors text-xs font-mono text-zinc-400">
            ...
            <div className="flex bg-zinc-900 rounded p-1 border border-zinc-800">
                <button onClick={() => setViewMode('list')} className={`p-1.5 rounded transition-all ${viewMode === 'list' ? 'bg-zinc-800 text-white shadow' : 'text-zinc-500 hover:text-zinc-300'}`}>
                    <List className="w-4 h-4" />
                </button>
                <button onClick={() => setViewMode('analytics')} className={`p-1.5 rounded transition-all ${viewMode === 'analytics' ? 'bg-primary/20 text-primary shadow' : 'text-zinc-500 hover:text-zinc-300'}`}>
        ...
        <div className="flex-1 overflow-auto p-4 md:p-6 space-y-6 pb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-surface/50 border border-tactical p-5 relative group hover:border-alert/30 flex flex-col justify-between">
               ...
               <button onClick={() => setIsModalOpen(true)} className="w-full mt-2 py-1.5 border border-tactical bg-zinc-900 hover:bg-zinc-800 flex items-center justify-center gap-2 text-[10px] font-mono text-zinc-300 font-bold uppercase tracking-wider"><Plus className="w-3 h-3" /> Input</button>
            </div>
          </div>
          ...
          {viewMode === 'list' ? (
            ...
            // --- MODO ANALYTICS (GRÁFICOS) ---
            <div className="space-y-6 animate-in fade-in duration-300">
                
                {/* 1. GRÁFICO DE EVOLUÇÃO (6 MESES) */}
                    ...
                    <div className="h-40 flex items-end justify-between gap-2 px-2">
                        {historyData.map((d, i) => (
                            <div key={i} className="flex-1 flex flex-col justify-end items-center gap-1 group relative">
                                ...
                                {/* Tooltip Simples */}
                                <div className="absolute bottom-full mb-2 bg-zinc-900 border border-zinc-700 p-2 rounded text-[10px] font-mono hidden group-hover:block z-10 whitespace-nowrap shadow-xl">
                                    <div className="text-emerald-500">In: {d.income}</div>
                                    <div className="text-red-500">Out: {d.expense}</div>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                {/* 2. CATEGORIAS + CUSTO FIXO (Lado a Lado) */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="border border-tactical bg-surface/30 p-5">
                        ...
                        <div className="space-y-3">
                            {sortedCategories.map((item: any) => {
                                ...
                                return (
                                <div key={item.cat}>
                                    <div className="flex justify-between items-end mb-1 text-[10px] uppercase text-zinc-400">
                                        <div className="flex items-center gap-1"><Icon className="w-3 h-3" /> {CATEGORY_LABELS[item.cat]}</div>
                                        <div>{item.val.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</div>
                                    </div>
                    ...
                    <div className="border border-tactical bg-surface/30 p-5">
                        <h3 className="text-xs font-bold tracking-widest text-zinc-400 uppercase flex items-center gap-2 mb-4"><Activity className="w-3 h-3" /> Fixos vs Variáveis</h3>
                         {/* Reutilizando lógica do passo anterior se necessário, ou simplificando aqui */}
                         <div className="text-center text-zinc-500 text-xs py-10 font-mono">DADOS DE ESTRUTURA EM BREVE</div>
                    </div>
                </div>
            </div>
          )}
        ...
        <TransactionModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSuccess={() => refreshAll()} />
      </main>
    </div>
  );
}
